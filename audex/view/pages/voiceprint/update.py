...
async def render(
    ...
    with ui.column().classes("w-full p-8 items-center"):
        ui.label("更新声纹").classes("text-h4 mb-4")

        with ui.card().classes("w-full max-w-2xl p-8 text-center"):
            ui.icon("fingerprint", size="4em").classes("text-warning mb-2")
            ui.label("功能开发中").classes("text-h6 mb-2")
            ui.label("声纹更新功能即将上线").classes("text-caption text-grey-7")
